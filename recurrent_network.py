...
class RNNetwork(object):
    ...
    def backward_pass(self,inputs,targets):
        ...
        dhback = [np.zeros_like(w) for w in self.weights_hidden]
        for i in reversed(range(0,len(inputs))):
            ...
            delta = half_delta*(1-self.hidden_states[i][-1]**2)
            ...
            dWh[-1]+=np.concatenate((np.dot(delta,self.hidden_states[i-1][-1].transpose()),\
                    np.dot(delta,self.hidden_states[i][-2]).transpose()),axis=1)

            for j in range(0,len(self.weights_hidden)):
